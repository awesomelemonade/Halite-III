...
public class GameConstants {
	...
	public static void populateConstants(final String stringFromEngine) {
		...
		if ((tokens.size() % 2) != 0) {
			DebugLog.log("Error: constants: expected even total number of key and value tokens from server.");
	...
	private static boolean getBoolean(final Map<String, String> map, final String key) {
		...
		case "false":
			return false;
		default:
			DebugLog.log("Error: constants: " + key + " constant has value of '" + stringValue
	...
	private static String getString(final Map<String, String> map, final String key) {
		if (!map.containsKey(key)) {
			DebugLog.log("Error: constants: server did not send " + key + " constant.");
			throw new IllegalStateException();
		}
		return map.get(key);
	}
